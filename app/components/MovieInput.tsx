...
export default function MovieInput({ onAnalyze, isLoading }: MovieInputProps) {
  ...
  // Accept any non-empty input (IMDb ID or movie name)
  ...
  return (
    <div className="w-full max-w-3xl mx-auto">
      ...
      >
        ...
        >
          ...
          <input
            ...
            placeholder="Enter IMDb ID or movie name"
            disabled={isLoading}
            className="
      ...
      >
        <span className="text-[11px] text-imdb-muted uppercase tracking-widest font-semibold">
          Quick picks:
        </span>
        {EXAMPLES.map((ex, i) => (
          <motion.button
          ...
          >
            <span>{ex.label}</span>
            <span className="text-imdb-muted group-hover:text-imdb-accent/60 transition-colors">
              '{ex.year.slice(2)}
            </span>
          </motion.button>
        ))}
